...
The command updates certain properties to prevent inconsistent visibilities with extended classes.

For example; if there is a `DataObject` containing `public static $db`, it will be updated to `private static $db`.

The rules for this is controlled in `.upgrade.yml`:
